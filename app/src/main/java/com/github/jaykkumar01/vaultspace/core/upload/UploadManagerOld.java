...
public final class UploadManagerOld {
    ...
    private List<MediaSelection> healRetryStoreIfNeeded(
        ...
        if (invalidCount > 0) {
            Log.w(
                    TAG,
                    "healRetryStoreIfNeeded(): groupId=" + albumId +
                            ", healed retries (valid=" + valid.size() +
                            ", dropped=" + invalidCount + ")"
            );

            retryStore.replaceAlbumRetries(albumId, valid);
            retryStore.flush();
        }
    ...
    public void registerObserver(
        ...
        List<MediaSelection> retryList = allRetries.get(albumId);
        if (retryList == null || retryList.isEmpty()) return;

        List<MediaSelection> validRetries =
                healRetryStoreIfNeeded(albumId, retryList);
    ...
    private void emitCancelled(@NonNull String albumId) {
        UploadObserver observer = observers.get(albumId);
        if (observer != null) {
    ...
    public void enqueue(
        ...
        Log.d(TAG, "enqueue(): groupId=" + albumId);
    ...
    void onUploadFailure() {
        ...
        String albumId = currentTask.albumId;
        ...
        if (retryable) {
            ...
            retryStore.addRetry(albumId, selection);
            retryStore.flush();
    ...
    public void cancelUploads(@NonNull String albumId) {
        queue.removeIf(task -> task.albumId.equals(albumId));

        boolean abortCurrent =
                currentTask != null &&
                        currentTask.albumId.equals(albumId);

        if (abortCurrent) {
            currentTask = null;
        }

        emitCancelled(albumId);
        uploadCache.removeSnapshot(albumId);

        retryStore.clearAlbum(albumId);
        retryStore.flush();
