 ...
 * - Session lifecycle
 */
public final class TrustedAccountsDriveHelper1 {
    ...
    public TrustedAccountsDriveHelper1(Context context) {
        ...
        executor.execute(() -> {
            ...
            try {
                ...
                if (rootFolderId == null) {
                    Log.d(TAG, "fetchTrustedAccounts: rootFolderId null");
                    postResult(callback, List.of());
                    return;
                }

                long tPermStart = SystemClock.elapsedRealtime();
                PermissionList permissions =
                        primaryDrive.permissions()
                                .list(rootFolderId)
                                .setFields("permissions(emailAddress,role,type)")
                                .execute();
                long tPermEnd = SystemClock.elapsedRealtime();

                Log.d(TAG, "Permissions fetch took "
                        + (tPermEnd - tPermStart) + " ms");
                ...
                for (Permission p : permissions.getPermissions()) {
                    ...
                    long tAccountStart = SystemClock.elapsedRealtime();
                    try {
                        Drive drive;
                        try {
                            drive = DriveClientProvider.forAccount(appContext, email);
                        } catch (Exception e) {
                            long tFail = SystemClock.elapsedRealtime();
                            Log.w(TAG, "Skipping trusted account (Drive init failed): "
                                    + email + " after " + (tFail - tAccountStart) + " ms | "
                        ...
                        try {
                            ...
                            long tOk = SystemClock.elapsedRealtime();
                            Log.d(TAG, "Storage fetch for " + email + " took "
                                    + (tOk - tAccountStart) + " ms");

                        } catch (Exception e) {
                            long tFail = SystemClock.elapsedRealtime();
                            Log.w(TAG, "Skipping trusted account (storage fetch failed): "
                                    + email + " after " + (tFail - tAccountStart) + " ms | "
                                    + e.getClass().getSimpleName() + ": " + e.getMessage());
                        }
                        ...
                        long tFail = SystemClock.elapsedRealtime();
                        Log.e(TAG, "Unexpected error for trusted account: " + email
                                + " after " + (tFail - tAccountStart) + " ms", unexpected);
                    }
                }

                long tEnd = SystemClock.elapsedRealtime();
                Log.d(TAG, "fetchTrustedAccounts TOTAL took "
                        + (tEnd - tStart) + " ms, accounts=" + result.size());

                postResult(callback, result);

            } catch (Exception e) {
                long tFail = SystemClock.elapsedRealtime();
                Log.e(TAG, "fetchTrustedAccounts FAILED after "
                        + (tFail - tStart) + " ms", e);
