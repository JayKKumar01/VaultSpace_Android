...
public final class DriveDataSource implements DataSource {
    ...
    private void readerLoop(long myGen, DriveStreamSource.StreamSession s) {
        ...
        try (InputStream in = s.stream()) {
            ...
            if (myGen == generation.get()) Log.e(TAG, "reader error gen=" + myGen, e);
            else Log.d(TAG, "reader obsolete gen=" + myGen);
        } finally {
            s.cancel();
            synchronized (lock) {
            ...
            Log.d(TAG, "READER END gen=" + myGen + " produced=" + producedBytes);
        }
    }
    ...
    private void invalidate() {
        ...
        if (closingGen != 0) {
            Log.d(TAG, "CLOSE gen=" + closingGen +
                    " open@" + openPosition +
